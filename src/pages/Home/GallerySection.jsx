...
function interleave(arrays) {
  const result = [];
  let index = 0;
  while (arrays.some(items => index < items.length)) {
    for (const items of arrays) if (index < items.length) result.push(items[index]);
    index += 1;
  }
  return result;
}

function productionGalleryItems() {
  return PRODUCTION_CASE_CATALOG.filter(entry => entry.status === 'production').flatMap(entry => entry.assets.map((asset, index) => ({
    id: `production-${entry.id}-${asset.id || index}`,
    type: 'visual',
    workType: 'visual',
    visualSkillId: asset.intent,
    title: asset.label,
    prompt: asset.prompt || productionPromptFor(asset.id),
    body_text: asset.prompt || productionPromptFor(asset.id),
    cover_url: asset.src,
    image_urls: [asset.src],
    ratio: asset.ratio,
    imageModel: 'image2',
    resolution: '2K',
    replay: {
      skillId: asset.intent,
      prompt: asset.prompt || productionPromptFor(asset.id),
      originalPrompt: asset.prompt || productionPromptFor(asset.id),
      imageModel: 'image2',
      ratio: asset.ratio,
      resolution: '2K',
      referenceAssets: [],
...
export default function GallerySection({ maxItems = 48, showHeader = true, onUseSameStyle }) {
  ...
  const galleryItems = useMemo(() => interleave([
    ecommerceCases,
    GALLERY,
