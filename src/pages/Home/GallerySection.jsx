...
function useGalleryColumnCount() {
  const [count, setCount] = useState(() => typeof window === 'undefined' ? 4 : window.innerWidth >= 1280 ? 4 : window.innerWidth >= 768 ? 3 : 2);
  useEffect(() => {
...
function TryOnWorkflowCard({ item, priority }) {
  const cards = tryOnWorkflowCards(item);
  return <div className="gallery-tryon-flow" aria-hidden="true">
    {cards.map((asset, assetIndex) => <React.Fragment key={asset.id || asset.url || assetIndex}>
      {assetIndex > 0 && <span className={`gallery-tryon-symbol symbol-${assetIndex}`}>{cards.length === 2 || assetIndex > 1 ? '→' : '+'}</span>}
...
export default function GallerySection({ maxItems = 200, showHeader = true, onUseSameStyle }) {
  ...
  const galleryColumns = useMemo(() => stableGalleryColumns(galleryItems.slice(0, itemLimit), columnCount), [columnCount, galleryItems, itemLimit]);
  ...
  return (
    <section className="gallery-section">
      {showHeader && <div className="gallery-heading"><div><div className="gallery-eyebrow">精选案例</div><h2>灵感发现</h2></div><span>{casesLoading ? `${galleryItems.length} 个案例 · 继续载入` : `${galleryItems.length} 个案例`}</span></div>}
      <div className="gallery-masonry" style={{ '--gallery-columns': columnCount }}>
        {galleryColumns.map((column, columnIndex) => <div className="gallery-masonry-column" key={`gallery-column-${columnIndex}`}>
        {column.filter(entry => entry.index < visibleItems.length).map(({ item, index }) => {
          const type = galleryType(item);
