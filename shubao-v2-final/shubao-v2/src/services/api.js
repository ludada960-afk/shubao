...
/* ── 电商图生成 ── */
export async function generateEcommerce({ productName, category, refImgs, styles, platform, points }) {
    ...
    if (res.ok) {
      ...
      try {
        ...
        const missing = local.filter(x =>
          x._saveKey != null && !seenKeys.has(String(x._saveKey)) && x.title && !x.title.includes('�')
        );
