...
async function contentAnalysis(textContent, refImageDescs = []) {
  ...
  const sysPrompt = CONTENT_ANALYSIS_SP.replace(/{{current_year}}/g, String(currentYear)).replace(/{{current_month}}/g, String(currentMonth)).replace(/{{text_content}}/g, enrichedText)
    .replace('{{detected_category}}', detectedCat || '');
  const userPrompt = CONTENT_ANALYSIS_UP
    .replace('{{category}}', '自动判断')
    .replace('{{text_content}}', enrichedText)
...
async function visualPlanning(analysisResult, refImageDescs = []) {
  ...
  const userPrompt = VISUAL_PLANNING_UP
    ...
    .replace('{{ref_image_hint}}', refImageHint)
    .replace(/{{current_year}}/g, String(currentYear))
    .replace(/{{current_month}}/g, String(currentMonth));
