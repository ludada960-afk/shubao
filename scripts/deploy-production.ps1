...
tar -czf $archive -C $repo `
  ...
  dist server scripts/nginx/shuimg.cn.conf scripts/check-ecommerce-idle.cjs package.json package-lock.json ecosystem.config.cjs ecosystem.production.cjs $galleryDirectoryName
...
try {
  ...
  Assert-DeploymentLockHeld
  Invoke-LockedRemote -Command "set -e; mkdir -p $remoteBackup; cp -a $RemoteDir/dist $remoteBackup/dist; mkdir -p $remoteBackup/server; rsync -a --delete --exclude='works.db' --exclude='works.db-shm' --exclude='works.db-wal' --exclude='generated-assets/' --exclude='uploads/' --exclude='temp_uploads/' --exclude='cache_img/' --exclude='cache_overlay/' --exclude='extension_downloads/' --exclude='extension_tasks/' --exclude='backups/' $RemoteDir/server/ $remoteBackup/server/; if [ -f $RemoteDir/server/works.db ]; then node $remoteDatabaseBackupHelper $RemoteDir $RemoteDir/server/works.db $remoteBackup/works.db; fi; if [ -L $WebRoot ] || [ -d $WebRoot ]; then sudo cp -aL $WebRoot $remoteBackup/webroot; else sudo cp -aL $legacyWebRoot $remoteBackup/webroot; fi; if [ -f $RemoteDir/ecosystem.production.cjs ]; then cp $RemoteDir/ecosystem.production.cjs $remoteBackup/ecosystem.production.cjs; fi; if pm2 describe shubao >/dev/null 2>&1; then legacy_pid=`$(pm2 pid shubao); case `"`$legacy_pid`" in ''|*[!0-9]*) exit 1 ;; esac; printf '%s\n' `"`$legacy_pid`" > $remoteBackup/legacy-pid; sha256sum $RemoteDir/server/index.mjs > $remoteBackup/legacy-server.sha256; fi; if [ -f $remotePm2ClusterMarker ]; then touch $remoteBackup/pm2-cluster-enabled; fi; sudo cp '$remoteNginxConfig' $remoteBackup/nginx-config" -TimeoutSeconds 600 -FailureMessage "Remote backup failed"
  $releaseStarted = $true
  Assert-DeploymentLockHeld
  Invoke-LockedRemote -Command "set -e; cd $RemoteDir; if [ -d '$RemoteDir/$galleryDirectoryName' ]; then mv '$RemoteDir/$galleryDirectoryName' '$remoteBackup/$galleryDirectoryName'; fi; tar xzf '$remoteReleaseArchive'; npm ci --omit=dev; mkdir -p $RemoteDir/.runtime; if [ -f $remotePm2ClusterMarker ]; then pm2 startOrReload ecosystem.production.cjs --only shubao-production --update-env; else pm2 delete shubao-production >/dev/null 2>&1 || true; pm2 start ecosystem.production.cjs --only shubao-production --update-env; touch $remotePm2ClusterMarker; fi; for attempt in `$(seq 1 60); do if curl -fsS http://127.0.0.1:3002/health; then break; fi; if [ `"`$attempt`" -eq 60 ]; then exit 1; fi; sleep 2; done; sudo mkdir -p $staticReleasesRoot $remoteStaticRelease; sudo cp -a $RemoteDir/dist/. $remoteStaticRelease/; sudo rm -f $remoteStaticNext; sudo ln -s $remoteStaticRelease $remoteStaticNext; sudo mv -Tf $remoteStaticNext $WebRoot; sudo cp '$RemoteDir/scripts/nginx/shuimg.cn.conf' '$remoteNginxConfig'; sudo nginx -t; sudo systemctl reload nginx" -TimeoutSeconds 2400 -FailureMessage "Remote restart or health check failed"
