...
try {
  ...
  Assert-DeploymentLockHeld
  $storagePreflightCommand = "set -e; mkdir -p $RemoteDir/deploy-backups; find $RemoteDir/deploy-backups -mindepth 1 -maxdepth 1 -type d -printf '%T@ %p\n' | sort -nr | tail -n +3 | cut -d' ' -f2- | xargs -r sudo rm -rf --; available_kb=`$(df -Pk $RemoteDir | awk 'END {print `$4}'); required_kb=2097152; case `"`$available_kb`" in ''|*[!0-9]*) echo 'Could not determine available production storage' >&2; exit 1 ;; esac; if [ `"`$available_kb`" -lt `"`$required_kb`" ]; then echo `"Insufficient production storage: `$available_kb KiB available, `$required_kb KiB required`" >&2; exit 1; fi"
  Invoke-LockedRemote -Command $storagePreflightCommand -TimeoutSeconds 300 -FailureMessage "Production storage preflight failed"
  Assert-DeploymentLockHeld
  Invoke-LockedRemote -Command "set -e; mkdir -p $remoteBackup; cp -a $RemoteDir/dist $remoteBackup/dist; if [ -f $RemoteDir/package.json ]; then cp $RemoteDir/package.json $remoteBackup/package.json; fi; if [ -f $RemoteDir/package-lock.json ]; then cp $RemoteDir/package-lock.json $remoteBackup/package-lock.json; fi; mkdir -p $remoteBackup/server; rsync -a --delete --exclude='works.db' --exclude='works.db-shm' --exclude='works.db-wal' --exclude='generated-assets/' --exclude='uploads/' --exclude='temp_uploads/' --exclude='video-assets/' --exclude='video-upload-staging/' --exclude='cache_img/' --exclude='cache_overlay/' --exclude='extension_downloads/' --exclude='extension_tasks/' --exclude='backups/' $RemoteDir/server/ $remoteBackup/server/; if [ -f $RemoteDir/server/works.db ]; then node $remoteDatabaseBackupHelper $RemoteDir $RemoteDir/server/works.db $remoteBackup/works.db; fi; if [ -L $WebRoot ] || [ -d $WebRoot ]; then sudo cp -aL $WebRoot $remoteBackup/webroot; else sudo cp -aL $legacyWebRoot $remoteBackup/webroot; fi; if [ -f $RemoteDir/ecosystem.production.config.cjs ]; then cp $RemoteDir/ecosystem.production.config.cjs $remoteBackup/ecosystem.production.config.cjs; fi; if pm2 describe shubao >/dev/null 2>&1; then legacy_pid=`$(pm2 pid shubao); case `"`$legacy_pid`" in ''|*[!0-9]*) exit 1 ;; esac; printf '%s\n' `"`$legacy_pid`" > $remoteBackup/legacy-pid; sha256sum $RemoteDir/server/index.mjs > $remoteBackup/legacy-server.sha256; fi; if [ -f $remotePm2ClusterMarker ]; then touch $remoteBackup/pm2-cluster-enabled; fi; sudo cp '$remoteNginxConfig' $remoteBackup/nginx-config" -TimeoutSeconds 600 -FailureMessage "Remote backup failed"
  $releaseStarted = $true
  Assert-DeploymentLockHeld
