...
test('XHS and both reachable Plog entrypoints use the shared ecommerce-point paywall without auto-saving prior forms', async () => {
  const xhs = await source('../src/pages/Home/XhsContentMode.jsx');
  const plog = await source('../src/pages/Plog/index.jsx');
  ...
  for (const [name, page] of [
    ...
    assert.match(page, /currency:\s*'ec_points'/, `${name} must select ecommerce points`);
