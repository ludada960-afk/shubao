...
test('uses the production-probed GPT-5.5 route as the default visual analysis model', async () => {
  ...
  assert.match(vlmClient, /model:\s*process\.env\.MINI_MODEL\s*\|\|\s*['"]gpt-5\.5['"]/);
  assert.match(server, /const MINI_MODEL\s*=\s*process\.env\.MINI_MODEL\s*\|\|\s*['"]gpt-5\.5['"]/);
  assert.match(envExample, /^MINI_MODEL=gpt-5\.5$/m);
