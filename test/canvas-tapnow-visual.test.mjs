...
/* 4c183cd4 续命 P-Canvas 中央弹窗 v3 视觉契约测试 (1 张参考图 + 1 句 prompt)
   用户 8-29 原话 (硬性):
   - "你直接去抄别人的面板是怎么做的呀? 就是进入画布的第一版视觉是怎么做的呀?" (用 TapNow 第一版视觉重做)
   - "我之前不是也一直跟你说, 你这个面板不能搞得这么宽吗?" (改窄)
   - "你为什么要展示这些告诉我的东西呢? ... 用户为什么要知道这个呢?" (不写内部术语)
   - "你下面这几个新添加的功能, 什么智能分镜套图还有配音这些, 我现在点击了都是没有反应的, 任何反应都没有啊" (修 4 智能按钮 bug, 空画布时也能开 composer)

   3 行分层 (资深美工 + 产品经理 + TapNow 调研 3 视角合一):
   - Row 1 添加素材 (3 入口): 上传图片 / 上传视频 / 从我的作品导入 — primary row
   - Row 2 AI 生成 (2 入口): 生成电商套图 / 生成视频 — generate row
   - Row 3 智能 (4 入口): 1-click 套图 / 1-click 视频 / TTS 配音 / 字幕动效 — smart row */
...
test('canvas-empty-actions.css declares the 3-row layered visual + glass + dark + responsive (4 子契约)', () => {
  ...
  assert.ok(css.indexOf('.ec-canvas-empty-row.is-smart-row') !== -1, 'Row 3 智能 必填 (4 入口)');
  // grid 配置
  ...
  assert.match(css, /\.is-smart-row\s*\{[\s\S]*grid-template-columns:\s*repeat\(4,/, 'Row 3 = 桌面 4 列');
  // 毛玻璃
...
test('EcCanvas/index.jsx 画布中央弹窗有 3 行分层 (1 添加素材 + 2 AI 生成 + 3 智能) 9 个按钮', () => {
  ...
  assert.ok(jsx.indexOf("actionId: 'caption-motion'") !== -1, 'Row 3 caption-motion 必填');
});

test('中央弹窗 4 智能按钮 走 addCanvasComposer (修 v2 bug: 空画布时 4 按钮无反应)', () => {
  /* 用户 8-29 原话: "你下面这几个新添加的功能, 什么智能分镜套图还有配音这些,
     ...
     v3 改用 addCanvasComposer (开 prompt 节点, 空画布时也能开) */
  const jsx = readFileSync(ecCanvasIndexPath, 'utf8');
  // 4 智能按钮 必须 addCanvasComposer 形式, 不准 handleCreateDerivedNode
  for (const actionId of ['one-click-suite', 'one-click-video', 'tts-voiceover', 'caption-motion']) {
