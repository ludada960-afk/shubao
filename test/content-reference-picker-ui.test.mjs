...
test('XHS and Plog pages use grouped reference assets instead of a single Plog image', async () => {
  ...
  assert.match(home, /XhsSupplementDeck/);
  assert.match(home, /EcommerceAddCard|EcommerceImageCard/);
  assert.match(home, /referenceAssets/);
  assert.match(standalone, /SupplementAssetDeck/);
  assert.match(standalone, /referenceAssets/);
  assert.match(home, /sourceImages\.length < 6/);
  assert.match(home, /styleImages\.length < 3/);
