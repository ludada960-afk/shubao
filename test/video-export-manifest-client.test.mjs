...
test('video export job client keeps the renderer handoff owner-scoped', async t => {
  const originalFetch = globalThis.fetch;
  const originalStorage = globalThis.localStorage;
  globalThis.localStorage = { getItem: () => JSON.stringify({ token: 'export-session' }) };
  const requests = [];
  globalThis.fetch = async (path, options = {}) => {
    requests.push({ path, options });
    const payload = path.includes('?') ? { jobs: [{ id: 'job-1' }] } : { job: { id: 'job-1' } };
    return new Response(JSON.stringify(payload), { status: options.method === 'POST' ? 202 : 200,
